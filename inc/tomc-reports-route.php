...
function getPayoutRecords($data){
    ...
    if (is_user_logged_in()){
        ...
        select users.display_name, sum(line_total.meta_value) as total_revenue, sum(stripe_fee) as stripe_fees, ((sum(line_total.meta_value) - sum(stripe.meta_value)) * (um.meta_value / 100)) as commission
        from %i completed 
        join %i items on completed.id = items.order_id
        and completed.status = "wc-completed"
        and items.order_item_type = "line_item"
        ...
        $results = $wpdb->get_results($wpdb->prepare($query, $orders_table, $order_items_table, $item_meta_table, $startDate, $endDate, $orders_table, $order_items_table, $item_meta_table, $startDate, $endDate, $orders_table, $order_items_table, $item_meta_table, $order_meta_table, $startDate, $endDate, $orders_table, $order_items_table, $item_meta_table, $item_meta_table, $startDate, $endDate), ARRAY_A);
        // return $results;
        return $wpdb->prepare($query, $orders_table, $order_items_table, $item_meta_table, $startDate, $endDate, $orders_table, $order_items_table, $item_meta_table, $startDate, $endDate, $orders_table, $order_items_table, $item_meta_table, $order_meta_table, $startDate, $endDate, $orders_table, $order_items_table, $item_meta_table, $item_meta_table, $startDate, $endDate);
    } else {
