...
function getPayoutRecords($data){
    ...
    if (is_user_logged_in()){
        ...
        $userId = $user->ID;
        $query = 'select users.display_name, sum(line_total.meta_value) as total_revenue, sum(stripe.meta_value) as stripe_fees, ((sum(line_total.meta_value) - sum(stripe.meta_value)) * (um.meta_value / 100))
        from %i completed 
        ...
        $results = $wpdb->get_results($wpdb->prepare($query, $orders_table, $order_items_table, $order_product_lookup_table, $posts_table, $users_table, $item_meta_table, $order_meta_table, $user_meta_table), ARRAY_A);
        return $results;
        // return 'success';
    } else {
